...
def upgrade() -> None:
    # Create enum type for task priority if it doesn't exist
    op.execute("DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'task_priority') THEN CREATE TYPE task_priority AS ENUM ('high', 'medium', 'low'); END IF; END $$")
